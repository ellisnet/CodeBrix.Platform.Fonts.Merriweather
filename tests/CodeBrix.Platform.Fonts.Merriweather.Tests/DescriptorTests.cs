...
{
    private const string PackageId = "CodeBrix.Platform.Fonts.Merriweather.OflLicenseForever";
    private const string PathPrefix = "ms-appx:///CodeBrix.Platform.Fonts.Merriweather/Fonts/";
    ...
    public void Descriptor_display_name_is_the_typographic_family_name()
        => Root().GetProperty("displayName").GetString().Should().Be("Merriweather");

    [Fact]
    public void Descriptor_resource_key_follows_the_family_convention()
        => Root().GetProperty("resourceKey").GetString().Should().Be("MerriweatherFont");
    ...
    {
        //Arrange — Greek, Armenian and Georgian are exactly the scripts
        //Merriweather itself does not carry.
        var names = FallbackUris().Select(Path.GetFileNameWithoutExtension).OrderBy(n => n).ToArray();

        //Assert
        names.Should().BeEquivalentTo(new[] { "NotoSerif", "NotoSerifArmenian", "NotoSerifGeorgian" });
    }
    ...
    {
        //Arrange — the companions exist precisely to add these three, so a
